...
class Exercise004Test extends AnyFlatSpec{
  ...
  "A correct date time" should "be returned when constructed with a date" in {
    ...
    assert(exercise004.getDateTimeWithGigaSecond.contains(LocalDateTime.of(2053, Month.MARCH, 27, 1, 46, 40)))
  }
  ...
  it should "be returned when constructed with a date and time" in {
    ...
    assert(exercise004.getDateTimeWithGigaSecond.contains(LocalDateTime.of(2052, Month.NOVEMBER, 11, 1, 8, 40)))
  }
  ...
  it should "be returned when constructed with a date and time with day roll over" in {
    ...
    assert(exercise004.getDateTimeWithGigaSecond.contains(LocalDateTime.of(2052, Month.OCTOBER, 3, 1, 46, 39)))
  }
  ...
  it should "be returned when the resulting date is a leap day" in {
    ...
    assert(exercise004.getDateTimeWithGigaSecond.contains(LocalDateTime.of(2020, Month.FEBRUARY, 29, 13, 46, 40)))
  }
  ...
  it should "be returned when both original and resulting date are BC" in {
    ...
    assert(exercise004.getDateTimeWithGigaSecond.contains(LocalDateTime.of(-1969, Month.SEPTEMBER, 9, 13, 46, 40)))
  }
  ...
  it should "be returned when original date is BC and resulting date is AC" in {
    ...
    assert(exercise004.getDateTimeWithGigaSecond.contains(LocalDateTime.of(16, Month.SEPTEMBER, 9, 13, 46, 40)))
  }
  ...
  it should "be handled correctly when resulting date greater than maximum allowed" in {
    ...
    assert(exercise004.getDateTimeWithGigaSecond.isEmpty)
  }
}
